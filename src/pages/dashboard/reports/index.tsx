...
const ReportsPage = () => {
  ...
  // Helper to get stats value (platform stats have .data wrapper, org stats don't)
  const getStatsValue = (platformKey: string, orgKey?: string) => {
    if (isAdmin && platformOrgStats?.data) {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      return (platformOrgStats.data as any)[platformKey] || 0;
    }
    if (isOrgOwner && orgOwnerStats) {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      return (orgOwnerStats as any)[orgKey || platformKey] || 0;
    }
    return 0;
  };
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-4">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {getStatsValue("totalOrganizations", "totalMembers")}
            </div>
            <p className="text-xs text-muted-foreground">
              {isAdmin ? "Active + Suspended" : "Total Members"}
            </p>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">
              {isAdmin ? "Active Trials" : "Active Members"}
            </CardTitle>
            <Calendar className="h-4 w-4 text-muted-foreground" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold">
              {getStatsValue("activeTrials", "activeMembers")}
            </div>
            <p className="text-xs text-muted-foreground">
              {isAdmin ? "Organizations on trial" : "Currently active"}
            </p>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">
              {isAdmin ? "Paid Plans" : "Total Teams"}
            </CardTitle>
            <TrendingUp className="h-4 w-4 text-muted-foreground" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold">
              {getStatsValue("paidOrganizations", "totalTeams")}
            </div>
            <p className="text-xs text-muted-foreground">
